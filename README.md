...
5. **設計** (`software-architect` → `design-reviewer`) — 実装設計書を作成する。
   ...
   - `design-reviewer` が要件定義書とリポジトリの実物に照らしてレビューする。観点: AC マッピングの網羅性、API 契約の具体性、影響を受けるファイルの実在と漏れ、既存の抽象化・レイヤリングへの適合、ステップの順序と粒度、リスクの見落とし、スコープの逸脱。要修正の指摘はアーキテクトの修正モードに渡されて再レビューされる（最大2回）。
