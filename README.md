...
- [Claude Opus 5 のプロンプティング](https://platform.claude.com/docs/ja/build-with-claude/prompt-engineering/prompting-claude-opus-5)（要件定義・設計エージェントとそのレビュアーが動くモデル）
...
- [Claude Fable 5.1 のプロンプティング](https://platform.claude.com/docs/ja/build-with-claude/prompt-engineering/prompting-claude-fable-5-1)（オーケストレーターは親セッションのモデルで動き、Opus 5 のことも Fable 5.1 のこともある）
...
- **検証指示を重ねない** — 「最終確認ステップを入れる」「回答を再確認する」「サブエージェントで検証する」といった指示は、Opus 5 が既に自前でやっていることと重複して過剰検証を招くので追加しない。検証は実コマンドを走らせる `implementer` / `test-engineer` の実作業としてのみ持たせる。
