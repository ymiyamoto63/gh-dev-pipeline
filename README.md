...
`requirements-review.md` / `design-review.md` / `implementation-notes.md` / `test-report.md` / `review-*.md` / `pr-description.md` は後続フェーズのための作業用の材料であり、意図的にローカル限定としている（要件・設計のレビュー結果は指摘が反映された文書そのものが Issue に載ることで記録され、コードレビューの4レポートは連結して Issue に投稿される）。
...
| `test-reviewer` | テスト | 変更した各振る舞いに対応するテストの有無、骨抜きテスト（アサーションなし・テスト対象のモック・出力のコピー・skip・握りつぶし）、既存テストの不当な弱体化、テストレポートの AC カバレッジ表と実テストコードの突き合わせ、テストの決定性 |
