...
この構成の帰結は2つある。クローンし直しても Issue だけで実行内容を再構成できる。リポジトリのコミット履歴に入るパイプラインの管理情報は、プリフライトで追記する `.gitignore` の1行と `lessons-learned.md` の追記だけなので、レビュー用の差分はこの2ファイルを除けばパイプラインが書いたコードとちょうど一致する。
