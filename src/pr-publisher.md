...
Before pushing, save the PR title and body you're about to submit to the path the caller specifies (default `<project_root>/.scratch/pr-description.md` if none given; create parent directories if they don't exist; overwrite if it already exists) so there's a record even if PR creation itself fails partway. Once `gh pr create` succeeds, delete this file — the PR body is now recorded on GitHub itself, so the local copy is redundant. It lives in the git-ignored scratch directory, so there is nothing to commit for its removal. Leave the file in place only if PR creation failed, so it still serves as a record of the intended content.
