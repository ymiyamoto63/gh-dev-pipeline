...
2. 受け入れ基準をカバーするテストを書く（または既存のテストを拡張する）。要件から示唆される現実的なエッジケースを含め、実際のスコープと無関係な網羅的な仮想エッジケースは含めないこと。テストの名前はリポジトリの命名規約に従い、検証する振る舞いを基準の文面から言い換えて付けること（例: 基準が「空のタイトルで保存すると400を返す」なら`returns 400 when title is empty`）。AC ID（AC-3 など）やIssue番号を、テスト名・describe/クラス名・コメント・タグ・フィクスチャ名に書き込まないこと — IDは要件定義書の改訂で振り直され、Issueごとに AC-1 から始まるためマージ後は何も指さなくなり、テストがリファクタされても誰も更新しないので、コードに残ったIDはやがて虚偽の対応関係になる。受け入れ基準とテストの対応はコードではなく、下記レポートのカバレッジ表が持つ。例外は、リポジトリの既存テストが既にトラッカーIDのタグ付けを規約として持っている場合で、そのときはその規約に従うこと。
...
- **受け入れ基準のカバレッジ**: 要件定義書の各受け入れ基準（AC-1, AC-2, …）を、それを検証した実物に対応付ける表。列は `AC | 検証方法 | 検証の実体 | 結果` とする。「検証の実体」列には、自動テストなら `テストファイルのパス::テスト名`（テスト名は describe/it の文字列やテストメソッド名をコードに書かれているとおりに写す。1つの基準を複数のテストで検証する場合はすべて列挙し、1つのテストが複数の基準を検証する場合は各行に同じテストを書く）、コマンド出力で検証したなら実行したコマンド、手動確認なら具体的なステップバイステップの手順（ツールで実行できる部分は実行し、その結果も書く）、検証できなかったならその理由を書く。この列がAC→テストの唯一の対応表である — テストコードにIDは書かないので、レビュアーはこの列だけを頼りにテストを開く。「ユニットテストでカバー」のような所在を特定できない記述は対応付けたことにならない。すべての基準がこの表に現れなければならない — 未検証の基準は漏れではなく発見事項である。各基準の検証タグを尊重すること: 自動テストの基準は実際に実行したテスト/コマンドで検証されなければならない。
